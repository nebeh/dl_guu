function  [xz,y,z] = peaksAdam(eta,gamma,beta,X,Y)


if nargin == 0
X = 2; Y = -2;  eta =  0.05; gamma = 0.9; beta = 0.5; 
elseif nargin == 1
    X = 2; Y = -2;  gamma = 0.9; beta = 0.5; 
elseif nargin == 2
    X = 2; Y = -2;  beta = 0.5; 
elseif nargin == 3
    X = 2; Y = -2;
...
while eps>1e-6 && i < 30
   % dx =- eta*dEdx(X,Y); dy =- eta*dEdy(X,Y);
    g21X = dEdx(X,Y).^2; g21Y = dEdy(X,Y).^2;
    EX = gamma*EX+(1-gamma)*g21X; EY = gamma*EY+(1-gamma)*g21Y;
    mx = beta*mx+(1-beta)*dEdx(X,Y);  my = beta*my+(1-beta)*dEdy(X,Y); 
    X1 = X- eta*mx/sqrt(EX+1e-16);
    Y1 = Y- eta*my/sqrt(EY+1e-16);
    eps = sqrt(abs(X-X1)+abs(Y-Y1));
    plot3([X,X1],[Y,Y1],[Z(X,Y),Z(X1,Y1)],'r-*')
